...
class InternalDataFacade final : public BaseDataFacade
{
  ...
  private:
    ...
    void LoadGeometries(const boost::filesystem::path &geometry_file)
    {
        ...
        util::SimpleLogger().Write(logWARNING) << " - bucket count: " << (number_of_indices-1)
                                               << ", compressed geometry count: " << number_of_compressed_geometries;
...
//        util::SimpleLogger().Write() << "get compressed for edge id " << id << ", size: " << (end-begin) ;
