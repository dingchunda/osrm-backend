...
#include "../protobuf/query-graph.pb.h"
...
#pragma pack(pop, r1)
    ...
    if (update_edge_weights || update_turn_penalties)
    {
        ...
        /* ==========================================================================================*/
//        std::vector<osrm::util::Coordinate> m_coordinate_list;
//        m_coordinate_list.resize(internal_to_external_node_map.size());
//        for (unsigned i = 0; i < internal_to_external_node_map.size(); i++) {
//            auto const n = internal_to_external_node_map[i];
//            m_coordinate_list[i] = osrm::util::Coordinate(n.lon, n.lat);
//        }

        pbrtree::Leaves pb_leaves;
        for (auto current_leaf = first; current_leaf != last; current_leaf++) {
            pbrtree::LeafNode *pb_leaf = pb_leaves.add_items();

            pb_leaf->set_itemcount(current_leaf->object_count);

            const osrm::util::RectangleInt2D &rectangle = current_leaf->minimum_bounding_rectangle;
            ...
            for (unsigned i = 0; i < current_leaf->object_count; i++) {
                auto &obj = current_leaf->objects[i];

                pbebg::EdgeBasedNode *pb_ebn = pb_leaf->add_items();
                extractor::EdgeBasedNode _n = (extractor::EdgeBasedNode)(obj);
                pb_ebn->set_u(_n.u);
                pb_ebn->set_v(_n.v);
        ...
        for (unsigned i = 0; i < m_geometry_indices.size()-1; i++) {
            unsigned index_first = m_geometry_indices[i];
            unsigned index_last = m_geometry_indices[i+1];

            pbnbg::EdgeBucket *pb_bucket = pb_cc.add_geometries();

            for (unsigned j = index_first; j < index_last; j++) {
                const auto &_e = m_geometry_list[j];

                pbnbg::CompressedEdge *pb_edge = pb_bucket->add_edges();
                pb_edge->set_node_id(_e.node_id);
                pb_edge->set_weight(_e.weight);
            };
...
void Contractor::WriteNodeLevels(std::vector<float> &&in_node_levels) const
{
    ...
    pbqg::NodeLevels pb_levels;

    for (unsigned i = 0; i < node_levels.size(); i++) {
        pb_levels.add_levels(node_levels[i]);
    }

    std::fstream pb_output("1.ch.levels.pb", std::ios::out | std::ios::binary);
    pb_levels.SerializeToOstream(&pb_output);
}
                                 ...
                                 const util::DeallocatingVector<QueryEdge> &contracted_edge_list)
{
    ...
    pbqg::QueryGraph pb_graph;

    pb_graph.set_v(max_node_id + 1);
    pb_graph.set_e(contracted_edge_count);
    ...
    for (const auto node : util::irange(0u, max_used_node_id + 1))
    {
        ...
        pb_graph.add_nodes()->set_first_edge(node_array[node].first_edge);
    }
         ...
         util::irange<unsigned>(max_used_node_id + 1, node_array.size()))
    {
        ...
        pb_graph.add_nodes()->set_first_edge(node_array[sentinel_counter].first_edge);
    }
    ...
    for (const auto edge : util::irange<std::size_t>(0UL, contracted_edge_list.size()))
    {
        ...
        if (contracted_edge_list[edge].target == contracted_edge_list[edge].source) {
            loopCount++;
        }

        pbqg::Edge *pb_edge = pb_graph.add_edges();
        pb_edge->set_target(current_edge.target);

        pbqg::EdgeData *pb_edge_data = pb_edge->mutable_data();
        pb_edge_data->set_id(current_edge.data.id);
        pb_edge_data->set_weight(current_edge.data.distance);
        pb_edge_data->set_shortcut(current_edge.data.shortcut);
        pb_edge_data->set_forward(current_edge.data.forward);
        pb_edge_data->set_backward(current_edge.data.backward);
    ...
    std::fstream pb_output("1.ch.graph.pb", std::ios::out | std::ios::binary);
