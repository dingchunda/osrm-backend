                             ...
                             double &ratio)
{
    ...
    printf("** query(wgs84): %d, %d\n", int32_t(query_location.lat), int32_t(query_location.lon));
    ...
    FloatCoordinate s = web_mercator::fromWGS84(segment_source);
    FloatCoordinate t = web_mercator::fromWGS84(segment_target);
    FloatCoordinate q = web_mercator::fromWGS84(query_location);

    nearest_location = web_mercator::toWGS84(projected_nearest);

    printf("** source(mercator): %.10f, %.10f\n", s.lat, s.lon);
    printf("** target(mercator): %.10f, %.10f\n", t.lat, t.lon);
    printf("** query(mercator): %.10f, %.10f\n", q.lat, q.lon);

    printf("** project(mercator): %.10f, %.10f\n", projected_nearest.lat, projected_nearest.lon);
    printf("** project(wgs84): %.10f, %.10f\n", nearest_location.lat, nearest_location.lon);
