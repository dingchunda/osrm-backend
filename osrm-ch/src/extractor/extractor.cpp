                                  ...
                                  const std::string &intersection_class_output_file)
{
    ...
    printf(" - NodeBasedGraph: node number: %d, edge number: %d\n", node_based_graph->GetNumberOfNodes(), node_based_graph->GetNumberOfEdges());
    for (const NodeID node_v : util::irange(0u, node_based_graph->GetNumberOfNodes())) {
        auto edge_count = node_based_graph->GetOutDegree(node_v);
        if (edge_count == 0) {
            printf(" * first node without edge found: %d\n", node_v);
            break;
        }
    }
    ...
    printf(" - After compress: node number: %d, edge number: %d\n", node_based_graph->GetNumberOfNodes(), node_based_graph->GetNumberOfEdges());
    printf(" - compressed edge count: %d\n", compressed_edge_container.BucketNumber());

    unsigned total = 0;
    unsigned reverse = 0;
    for (const NodeID node_v : util::irange(0u, node_based_graph->GetNumberOfNodes())) {
        for (auto current_edge : node_based_graph->GetAdjacentEdgeRange(node_v)) {
            total++;

            util::NodeBasedDynamicGraph::EdgeData &edge_data = node_based_graph->GetEdgeData(current_edge);
            if (edge_data.reversed) {
                reverse++;
            }
        }
    }
    printf(" - total: %d, reverse: %d\n", total, reverse);
    ...
    for (auto index : util::irange<std::size_t>(0UL, node_is_startpoint.size()))
    {
        ...
        if (node_is_startpoint[index])
        {
            *out_iter = *in_iter;
            ...
            printf("  - drop non-startpoint: %d, %d\n", std::uint32_t(in_iter->forward_segment_id.id), std::uint32_t(in_iter->reverse_segment_id.id));
        }
        in_iter++;
    }
...
//    pbebg::EdgeBasedNodeList _ebnlist;
//    for (const auto &ebn : node_based_edge_list) {
//        pbebg::EdgeBasedNode *_ebn = _ebnlist.add_nodes();
//        _ebn->set_u(ebn.u);
//        _ebn->set_v(ebn.v);
//        _ebn->set_forward_enabled(ebn.forward_segment_id.enabled);
//        _ebn->set_reverse_enabled(ebn.reverse_segment_id.enabled);
//        _ebn->set_forward_segment_id(ebn.forward_segment_id.id);
//        _ebn->set_reverse_segment_id(ebn.reverse_segment_id.id);
//        _ebn->set_forward_packed_geometry_id(ebn.forward_packed_geometry_id);
//        _ebn->set_reverse_packed_geometry_id(ebn.reverse_packed_geometry_id);
//        _ebn->set_fwd_segment_position(ebn.fwd_segment_position);
//        _ebn->set_name_id(ebn.name_id);
//        _ebn->set_component_id(ebn.component.id);
//        _ebn->set_is_tiny(ebn.component.is_tiny);
//    }
//    std::fstream _ebnlist_out("1.ebg.nodes.pb", std::ios::out | std::ios::binary);
//    _ebnlist.SerializeToOstream(&_ebnlist_out);
    ...
    util::DeallocatingVector<EdgeBasedEdge> const &edge_based_edge_list)
{
    ...
    pbebg::EdgeBasedGraph pb_ebg;
    pb_ebg.set_count(number_of_used_edges);

    for (const auto &edge : edge_based_edge_list)
    {
        file_out_stream.write((char *)&edge, sizeof(EdgeBasedEdge));

        pbebg::EdgeBasedEdge *pb_ebe = pb_ebg.add_edges();
        pb_ebe->set_source(edge.source);
        pb_ebe->set_target(edge.target);
        pb_ebe->set_edge_id(edge.edge_id);
        pb_ebe->set_weight(edge.weight);
        pb_ebe->set_forward(edge.forward);
        pb_ebe->set_backward(edge.backward);
    }

    std::fstream pb_out("1.ebg.edges.pb", std::ios::out|std::ios::binary);
    pb_ebg.SerializeToOstream(&pb_out);
