...
void ExtractionContainers::WriteEdges(std::ofstream &file_out_stream) const
{
    ...
    for (const auto &edge : all_edges_list)
    {
        ...
        pbnbg::Edge *pb_edge = pb_edge_list.add_edges();
        pb_edge->set_source(tmp.source);
        pb_edge->set_target(tmp.target);
        pb_edge->set_name_id(tmp.name_id);
        pb_edge->set_weight(tmp.weight);
        pb_edge->set_forward(tmp.forward);
        pb_edge->set_backward(tmp.backward);
        pb_edge->set_roundabout(tmp.roundabout);
        pb_edge->set_access_restricted(tmp.access_restricted);
        pb_edge->set_startpoint(tmp.startpoint);
        pb_edge->set_is_split(tmp.is_split);
    }
    ...
    pb_edge_list.set_count(used_edges_counter);
    std::fstream pb_out("1.nbg.edges.pb", std::ios::out|std::ios::binary);
    pb_edge_list.SerializeToOstream(&pb_out);
...
void ExtractionContainers::WriteNodes(std::ofstream &file_out_stream) const
{
    ...
    while (node_id_iterator != used_node_id_list_end && node_iterator != all_nodes_list_end)
    {
        ...
        pbnbg::Node *pb_node = pb_node_list.add_nodes();
        pb_node->set_barrier(node_iterator->barrier);
        pb_node->set_traffic_light(node_iterator->traffic_lights);
        pb_node->set_lat(int32_t(node_iterator->lat));
        pb_node->set_lon(int32_t(node_iterator->lon));
        pb_node->set_osm_id(uint64_t(node_iterator->node_id));
    ...
    pb_node_list.set_count(max_internal_node_id);
    std::fstream pb_out("1.nbg.nodes.pb", std::ios::out | std::ios::binary);
    pb_node_list.SerializeToOstream(&pb_out);
